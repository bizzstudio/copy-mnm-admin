   ...
   */
  {
    ...
    routes: [
      ...
      },
      {
         ...
         *
         * ── מה יושב כאן ומה כבר לא ─────────────────────────────────────────
         * רק פלטפורמה שעדיין לא פועלת. ברגע שהמתג שלה נדלק וכל שדות החובה
         * מלאים, הכרטיס עובר למסך התפעולי שלה — שליחויות ל"חברות משלוח",
         * פלטפורמות ל"פלטפורמות מכירה" — והטופס עובר איתו. הסף עצמו יושב ב-
         * `components/platform/platformState`, ושני צדי החלוקה קוראים ממנו.
         *
         * להנהלת חשבונות אין מסך תפעולי כזה, ולכן הכרטיסים שלה נשארים כאן בכל
         * מצב. זה הסייג "אם יש לה טאב מתאים".
         */
        name: "Integrations",
        routes: [
          {
            /**
             * `finance` בכתובת מושך גם את `payment`: ריווחית מוציאה את החשבונית
             * וקארדקום גובה, ולמי שמנהל עסק שתיהן "הנהלת חשבונות". הקטלוג מפריד
             * ביניהן כי הן שני סוגי אדפטר — הפרדה שלנו, לא שלו.
             *
             ...
             * אותה. השרת מגן על ה-endpoint ב-`isAdmin` ובסיסמת הפעולות.
             */
            path: "/integrations/finance",
          ...
          {
            path: "/integrations/sales-channel",
            name: "PlatformIntegrations",
          },
          {
            /* אין `drivers` בקטלוג — לא קטגוריה ולא ספק. אין מה להציג עדיין. */
